System.register(["./preact-app-legacy-D-e9SRIZ.js"],(function(e,t){"use strict";var r,o,a,n,s,i;return{setters:[e=>{r=e.d,o=e.A,a=e.h,n=e._,s=e.l,i=e.g}],execute:function(){function d(){const[e,t]=r(""),[n,d]=r(""),[l,u]=r(!1),[c,g]=r("");r((()=>{const e=new URLSearchParams(window.location.search);e.has("error")?g("Invalid username or password"):e.has("auth_required")&&g("Authentication required. Please log in to continue.")}));const m=o(null);return a`
    <section id="login-page" class="page flex items-center justify-center min-h-screen">
      <div class="login-container w-full max-w-md p-6 bg-white dark:bg-gray-800 rounded-lg shadow-lg">
        ...
        <form id="login-form" class="space-y-6" action="/api/auth/login" method="POST" onSubmit=${async t=>{if(t.preventDefault(),!e||!n)return void g("Please enter both username and password");u(!0),m.current=s();const r=btoa(`${e}:${n}`);localStorage.setItem("auth",r);try{const t=await i("/api/auth/login",{method:"POST",headers:{"Content-Type":"application/json",Authorization:"Basic "+r,"X-Requested-With":"XMLHttpRequest",Accept:"application/json"},body:JSON.stringify({username:e,password:n}),credentials:"include",mode:"same-origin",signal:m.current?.signal,timeout:5e3,retries:1,retryDelay:1e3});t.ok||302===t.status?window.location.href="/index.html?t="+(new Date).getTime():(u(!1),g("Invalid username or password"),localStorage.removeItem("auth"))}catch(o){console.error("Login error:",o),"Request timed out"===o.message&&localStorage.getItem("auth")?(console.log("Login request timed out, proceeding with stored credentials"),window.location.href="/index.html?t="+(new Date).getTime()):localStorage.getItem("auth")?(console.log("Login API error, but proceeding with stored credentials"),window.location.href="/index.html?t="+(new Date).getTime()):(u(!1),g("Login failed. Please try again."))}}}>
  ...
  `}e({LoginView:d,loadLoginView:function(){const e=document.getElementById("main-content");e&&n((async()=>{const{render:e}=await t.import("./preact-app-legacy-D-e9SRIZ.js").then((e=>e.p));return{render:e}}),void 0,t.meta.url).then((({render:t})=>{t(a`<${d} />`,e)}))}})}}}));
//# sourceMappingURL=LoginView-legacy-BuJeQNSq.js.map
