...
if [ -n "${CMAKE_TOOLCHAIN_FILE:-}" ]; then
    TOOLCHAIN_OPTION=("-DCMAKE_TOOLCHAIN_FILE=$CMAKE_TOOLCHAIN_FILE")
    echo "Using CMake toolchain: $CMAKE_TOOLCHAIN_FILE"
fi

cmake -DCMAKE_BUILD_TYPE="$BUILD_TYPE" $SOD_OPTION $TEST_OPTION "${GO2RTC_OPTIONS[@]}" $HTTP_BACKEND_OPTION $FFMPEG_CMAKE_OPTIONS "${TOOLCHAIN_OPTION[@]}" \
