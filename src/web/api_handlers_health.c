...
static void *health_check_thread_func(void *arg) {
    ...
    while (g_health_thread_running) {
        ...
        // Check if the web server thread is still running
        if (g_web_server_thread_id_set && !is_web_server_thread_running()) {
            log_error("Web server thread has crashed or stopped unexpectedly");

            // Mark for restart
            g_failed_health_checks += 3; // Immediately trigger restart threshold
            g_is_healthy = false;
            mark_server_for_restart();

            // Attempt to restart the web server directly from the health check thread
            if (restart_web_server()) {
                log_info("Web server restarted successfully after crash detection");
                reset_server_restart_flag();
                reset_health_metrics();
            } else {
                log_error("Failed to restart web server after crash detection");
            }
        }
        ...
        // If health check failed and server needs restart, try to restart it
        if (!check_result && check_server_restart_needed()) {
            log_info("Health check failed and server needs restart");

            // Attempt to restart the web server directly from the health check thread
            if (restart_web_server()) {
                log_info("Web server restarted successfully after health check failure");
                reset_server_restart_flag();
                reset_health_metrics();
            } else {
                log_error("Failed to restart web server after health check failure");
            }
        }
...
static bool is_web_server_thread_running(void) {
    if (!g_web_server_thread_id_set) {
        // Thread ID not set, assume it's not running
        return false;
    }

    pthread_mutex_lock(&g_web_server_thread_mutex);
    pthread_t thread_id = g_web_server_thread_id;
    pthread_mutex_unlock(&g_web_server_thread_mutex);

    // Send signal 0 to check if the thread exists
    ...
    if (result == 0) {
        ...
        log_warn("Web server thread (ID: %lu) no longer exists", (unsigned long)thread_id);
        return false;
    } else {
        // Some other error
        log_error("Error checking web server thread status: %s", strerror(result));
        return false;
    }
}
 ...
 */
static bool restart_web_server(void) {
    extern http_server_handle_t http_server; // Reference to the global server handle

    if (!http_server) {
        log_error("Cannot restart web server: server handle is NULL");
        return false;
    }

    log_info("Attempting to restart web server");
