...
void mg_handle_hls_master_playlist(struct mg_connection *c, struct mg_http_message *hm) {
    if (!c || !hm) {
        log_error("Invalid parameters in mg_handle_hls_master_playlist");
        return;
    }
    ...
    if (storage_path_hls[0] != '\0') {
        snprintf(hls_file_path, sizeof(hls_file_path), "%s/hls/%s/index.m3u8",
                storage_path_hls, decoded_stream_name);
        log_info("Using HLS-specific storage path: %s", storage_path_hls);
    } else {
        snprintf(hls_file_path, sizeof(hls_file_path), "%s/hls/%s/index.m3u8",
                storage_path, decoded_stream_name);
...
void mg_handle_hls_media_playlist(struct mg_connection *c, struct mg_http_message *hm) {
    if (!c || !hm) {
        log_error("Invalid parameters in mg_handle_hls_media_playlist");
        return;
    }
...
void mg_handle_hls_segment(struct mg_connection *c, struct mg_http_message *hm) {
    if (!c || !hm) {
        log_error("Invalid parameters in mg_handle_hls_segment");
        return;
    }
    ...
    if (!segment_filename || *segment_filename == '\0') {
        log_error("Empty segment filename in URI: %s", uri);
        mg_http_reply(c, 400, "", "{\"error\": \"Invalid URI format - empty segment filename\"}\n");
        return;
    }
    ...
    if (storage_path_hls[0] != '\0') {
        snprintf(hls_file_path, sizeof(hls_file_path), "%s/hls/%s/%s",
                storage_path_hls, decoded_stream_name, segment_filename);
        log_info("Using HLS-specific storage path: %s", storage_path_hls);
    } else {
        snprintf(hls_file_path, sizeof(hls_file_path), "%s/hls/%s/%s",
                storage_path, decoded_stream_name, segment_filename);
        log_info("Using default storage path for HLS: %s", storage_path);
    }
