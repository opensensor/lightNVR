...
char* create_json_string(const config_t *config) {
    ...
    cJSON_AddNumberToObject(json, "max_storage", (double)config->max_storage_size / (double)(1024ULL * 1024ULL * 1024ULL)); // Convert to GB
    ...
    cJSON_AddNumberToObject(json, "swap_size", (double)config->swap_size / (double)(1024ULL * 1024ULL)); // Convert to MB
