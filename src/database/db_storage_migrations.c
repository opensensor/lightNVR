...
static void populate(sqlite3_stmt *statement, storage_migration_job_t *job) {
    ...
    copy_column(job->destination_target_uuid,
                sizeof(job->destination_target_uuid), statement, 5);
    copy_column(job->destination_object_key,
                sizeof(job->destination_object_key), statement, 6);
    copy_column(job->state, sizeof(job->state), statement, 7);
    copy_column(job->checksum, sizeof(job->checksum), statement, 8);
    job->bytes_total = (uint64_t)sqlite3_column_int64(statement, 9);
    job->bytes_copied = (uint64_t)sqlite3_column_int64(statement, 10);
    job->attempt_count = sqlite3_column_int(statement, 11);
    job->max_attempts = sqlite3_column_int(statement, 12);
    job->next_attempt_at = sqlite3_column_int64(statement, 13);
    copy_column(job->last_error, sizeof(job->last_error), statement, 14);
    job->revision = sqlite3_column_int64(statement, 15);
    job->created_at = sqlite3_column_int64(statement, 16);
    job->updated_at = sqlite3_column_int64(statement, 17);
    job->started_at = sqlite3_column_int64(statement, 18);
    job->completed_at = sqlite3_column_int64(statement, 19);
}
...
db_storage_migration_result_t db_storage_migration_create(
    ...
    if (recording_id == 0 || owner_user_id < 0 || !job ||
        !lightnvr_uuid_is_valid(destination_target_uuid)) {
    ...
    const char *target_sql =
        "SELECT enabled,health_status,root_path FROM storage_targets "
        "WHERE uuid=? LIMIT 1;";
    ...
    const char *insert_sql =
        ...
        "uuid,recording_id,owner_user_id,source_target_uuid,"
    ...
    if (result == SQLITE_OK) {
        ...
        sqlite3_bind_text(statement, 4, source_target, -1, SQLITE_TRANSIENT);
        sqlite3_bind_text(statement, 5, source_key, -1, SQLITE_TRANSIENT);
        sqlite3_bind_text(statement, 6, destination_target_uuid, -1,
                          SQLITE_TRANSIENT);
        sqlite3_bind_text(statement, 7, source_key, -1, SQLITE_TRANSIENT);
        sqlite3_bind_int64(statement, 8, (sqlite3_int64)bytes_total);
        result = sqlite3_step(statement);
    }
    if (statement) sqlite3_finalize(statement);
...
int db_storage_migration_claim_due(storage_migration_job_t *job) {
    ...
    const char *select_sql = "SELECT " MIGRATION_SELECT_FIELDS
        ...
        "(j.attempt_count<j.max_attempts AND ("
        "j.state IN('queued','copying','verifying','committing') OR "
        "(j.state='retry_wait' AND j.next_attempt_at<=strftime('%s','now')))) "
