...
uint64_t add_stream_config(const stream_config_t *stream) {
    ...
    const char *sql = "INSERT INTO streams (name, url, enabled, streaming_enabled, width, height, fps, codec, priority, record, segment_duration, "
          ...
          "detection_record_on_schedule, detection_recording_schedule, camera_uuid, location_uuid) "
          "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
...
int get_stream_config_by_name(const char *name, stream_config_t *stream) {
    ...
    enum {
        ...
        COL_CAMERA_UUID, COL_LOCATION_UUID
    };
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        // Secondary detection stream URL
        const char *detection_url_val = (const char *)sqlite3_column_text(stmt, COL_DETECTION_URL);
        if (detection_url_val) {
            safe_strcpy(stream->detection_url, detection_url_val, sizeof(stream->detection_url), 0);
        } else {
            stream->detection_url[0] = '\0';
        }

        // RTMP/RTMPS publish (restream) destination URL
        const char *publish_url_val = (const char *)sqlite3_column_text(stmt, COL_PUBLISH_URL);
        if (publish_url_val) {
            safe_strcpy(stream->publish_url, publish_url_val, sizeof(stream->publish_url), 0);
        } else {
...
int get_all_stream_configs(stream_config_t *streams, int max_count) {
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW && count < max_count) {
        ...
        // Secondary detection stream URL
        const char *det_url = (const char *)sqlite3_column_text(stmt, COL_DETECTION_URL);
        if (det_url) {
            safe_strcpy(s->detection_url, det_url, sizeof(s->detection_url), 0);
        } else {
            s->detection_url[0] = '\0';
        }

        // RTMP/RTMPS publish (restream) destination URL
        const char *pub_url = (const char *)sqlite3_column_text(stmt, COL_PUBLISH_URL);
        if (pub_url) {
            safe_strcpy(s->publish_url, pub_url, sizeof(s->publish_url), 0);
        } else {
            s->publish_url[0] = '\0';
        }
